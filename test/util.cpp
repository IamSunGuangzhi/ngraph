...
{
    auto outputs = ngraph::get_subgraph_outputs(OutputVector{}, NodeVector{});
    ...
    outputs = ngraph::get_subgraph_outputs(OutputVector{A}, NodeVector{});
    EXPECT_EQ(outputs, (OutputVector{A}));

    outputs = ngraph::get_subgraph_outputs(OutputVector{A}, NodeVector{A});
    EXPECT_EQ(outputs, (OutputVector{}));

    outputs = ngraph::get_subgraph_outputs(OutputVector{A, absn}, NodeVector{});
    EXPECT_EQ(outputs, (OutputVector{absn}));

    auto B = make_shared<op::Parameter>(element::f32, shape);
    auto abs_b = make_shared<op::Abs>(B);
    auto neg_b = make_shared<op::Negative>(B);
    auto abs_b_neg = make_shared<op::Negative>(abs_b);
    outputs = ngraph::get_subgraph_outputs(OutputVector{B, abs_b}, NodeVector{});
    EXPECT_EQ(outputs, (OutputVector{B, abs_b}));

    outputs = ngraph::get_subgraph_outputs(OutputVector{B, abs_b}, NodeVector{B});
    EXPECT_EQ(outputs, (OutputVector{abs_b}));

    outputs = ngraph::get_subgraph_outputs(OutputVector{B, abs_b, abs_b_neg}, NodeVector{});
    EXPECT_EQ(outputs, (OutputVector{B}));

    auto add_b = make_shared<op::Add>(neg_b, abs_b_neg);
    outputs =
        ngraph::get_subgraph_outputs(OutputVector{B, abs_b, neg_b, abs_b_neg, add_b}, NodeVector{});
    EXPECT_EQ(outputs, (OutputVector{}));

    // now add_b uses abs_b_neg
    outputs = ngraph::get_subgraph_outputs(OutputVector{B, abs_b, abs_b_neg}, NodeVector{});
    EXPECT_EQ(outputs, (OutputVector{B, abs_b_neg}));
}
